...
/// Zero-copy decoder for the batch processor input.
...
impl<'a> Decoder<'a> {
    /// Decodes the batch processor input from a raw byte buffer.
    ///
    /// # Panics
    ...
    /// Returns a zero-copy multi-proof view borrowing from the input buffer.
