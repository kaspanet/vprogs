...
//! chain forward from the covenant's deploy block, finds the NEWEST settlement of this covenant, and
//! reconstructs the live continuation [`CovenantState`] from it deterministically (the same way the
//! bridge derives `last_settlement`). The worker then bounded-confirms that outpoint; a competitor
//! advancing the covenant between the scan and the confirm just triggers a re-scan for the newer
//! tip. A covenant that has never settled (the scan finds nothing) folds into the same bounded
//! re-resolve loop against the still-unspent bootstrap, so a competitor settling it out from under
//! the scan is recovered by the re-scan rather than a panic.
...
/// a settlement off a block being removed. The `min_confirmation_count` floor is left unset, matching
/// the bridge, whose adaptive `ReorgFilter` (`l1/bridge/src/reorg_filter.rs`) reports no threshold
/// until it has actually observed a reorg - there is no running history at startup, and a fixed
/// positive floor would trim the recent tip, so under continuous settlement (the live tip always
/// within the window) the scan would keep returning an already-spent older settlement and the
/// caller's bounded re-resolve loop could never confirm. The caller's loop is the backstop for a tip
/// resolved off a branch that is still settling out: it re-scans whenever the resolved tip's UTXO is
/// already spent.
        ...
        loop {
            ...
            for chain_block in response.chain_block_accepted_transactions.iter() {
                ...
                for rpc_tx in &chain_block.accepted_transactions {
                    ...
                    if let Some(info) = tx.settlement_info(covenant_id, block_hash) {
                        newest = Some(info);
                    }
                }
                cursor = block_hash;
