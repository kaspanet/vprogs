...
//! (`control_id`, `hashfn`) extracted from a succinct receipt — the host needs them at
//! redeem-build time to bake the right pins into the script. The Groth16 covenant variant
//! doesn't need this: its identity constants are build-time consts in the covenant crate.
...
/// into [`vprogs_zk_backend_risc0_covenant::SuccinctPins::control_id`] / `hashfn` (and the
/// matching bootstrap fields) so the redeem script bakes the same values the prover used.
///
/// The Groth16 covenant variant does not need this — Groth16 identity constants live in the
/// covenant crate's `groth16_consts` module (generated by its build.rs).
