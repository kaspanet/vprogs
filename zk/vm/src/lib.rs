...
/// ZK VM that executes guest programs via a [`ZkBackend`] and optionally sends proof requests
...
impl<B: ZkBackend> ZkVm<B> {
    /// Creates a new ZK VM with the given backend and guest ELF binary.
...
impl<B: ZkBackend> VmInterface for ZkVm<B> {
    fn process_transaction<S: Store<StateSpace = StateSpace>>(
        ...
        // 2. Execute the guest (no proof generation, just execution).
