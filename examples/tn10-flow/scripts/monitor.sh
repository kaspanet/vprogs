...
PAT_CATCHUP='catching up to existing covenant|bootstrap covenant .* confirmed'
...
#   B (catchup):              "catching up to existing covenant <64hex>"
covenant_id() { # covenant_id <log>
  grep -aoE 'covenant ([0-9a-f]{64}) ready|catching up to existing covenant ([0-9a-f]{64})' "$1" 2>/dev/null \
