...
- **Hard-code in the guest**: not robust against parameter changes (which Kaspa may legitimately make over time — hardforks, soft tuning, etc.). A guest hard-coded with `F = 86400` becomes wrong the moment Kaspa rotates the value. Re-versioning the guest is not a way out: a covenant pins one guest verification key, so state locked under the old guest can't be spent by a new version. The guest must stay valid across rotations — i.e., not bake `F` in.
...
- **Carry it as a public input to the proof**: unsound absent something to check it against — an attacker could pass a different `F` value and the guest would accept it. It *could* be made sound by verifying the input against an on-chain commitment to the parameter, but Kaspa exposes no such parameter commitment today.
...
("Latest" = closest to B on the selected-parent chain; blue_score strictly increases along that chain, so this is equivalently the satisfying ancestor with maximal blue_score. "Proper ancestor" excludes B itself. "Non-genesis" excludes the genesis block even when it would otherwise qualify; see sentinel rationale below. Strict `<`.)
...
Guest verifiers treat `ZERO_HASH` as the walk-termination marker uniformly; they don't distinguish the two sentinel cases and don't need to know `H_daa`. This sentinel use relies on the existing property that no valid block's `seq_commit` equals `ZERO_HASH` — a domain-separated hash meets the all-zero value only with negligible probability — and the proposal adds no new rule around it.
...
**Trust boundary (L1 vs guest)**: the guest cryptographically verifies *which form* was committed — reset or continuation — by recomputing the emitted `lane_tip` (§4.3). What the guest does *not* independently verify is whether L1 was semantically *justified* in choosing that form, nor whether the shortcut target is the latest satisfying ancestor under §2.2's predicate (the guest doesn't know `F` at any block). L1 consensus is responsible for that semantic correctness; valid chain blocks are produced only when I1–I4 hold. The guest verifies only that the *committed values* used in the proof (the shortcut target, the lane_tip, the lanes_root) are the values L1 actually bound into the block's `seq_commit` (via `activity_root` and `seq_state_root`, §2.1). This division is what lets the guest stay free of `F` (and other consensus parameters) entirely.

**Security assumption (explicit)**: soundness rests on L1 consensus producing only blocks that satisfy I1–I4, plus the existing hash-domain separation behind the `ZERO_HASH` sentinel (§2.2). Since the guest checks only hash consistency (per the trust boundary above), a consensus bug in shortcut computation or eviction semantics is a soundness bug for these L2 inactivity proofs, not merely an L1 issue.

These four invariants plus the trust boundary are sufficient for a guest to verify lane inactivity over post-activation spans under the proof scope described in §5 and §8, **without knowing the value of `F`**. The guest verifies each step cryptographically against the structural commitments.
...
This verifier proves reset-form emission and inactivity only. It does *not* validate that R's `activity_digest` faithfully represents the lane's published transactions — that is the separate activity/payload verification path; here `activity_digest` enters only as an opaque input bound through `lane_tip_next` and the seq_commit fold (§4.3).
...
Every full anchor is implicitly post-activation: `seq_commit` here applies the `activity_root_hash` wrap (§2.1), so a pre-activation block fails recomposition — enforcing activation without the guest knowing `H_daa` (§4.2, §8).
...
§§5.0–5.6 prove soundness and completeness for the **constant-F regime** (`F(B) = F` for all `B`), which is Kaspa's current configuration. Under constant `F` the §2.2 predicate `A.bs + min(F(A), F(B)) < B.bs` collapses to `A.bs + F < B.bs`, and references to `F` in these subsections mean that constant value. §5.7 addresses dynamic `F` under a rotation-spacing assumption (consecutive rotations separated by more than the larger `finality_depth` across the change), arguing that dynamic-`F` walks decompose into constant-`F` segments plus isolated single-boundary transitions; denser schedules are out of scope.
...
The chain block L (where the lane was last active) has `L.blue_score = L_lane_blue_score`. Assume `L` is non-genesis (the practical case — non-coinbase lanes typically have no activity at genesis) and in-domain (`L.daa_score ≥ H_daa`; §8 scopes this format to post-activation L). As long as the current walked block W's `blue_score > L_lane_blue_score + F`, L itself satisfies the `inactivity_shortcut` predicate (`L.blue_score + F < W.blue_score`) *and* is a non-genesis in-domain ancestor, so L is in the candidate set for `inactivity_shortcut(W)`, and the "latest such candidate" picks something at L's blue_score or higher.

Therefore the walk never overshoots L: each `W_(i+1).blue_score ≥ L_lane_blue_score`. (Edge case for `L = genesis`: §2.2's genesis exclusion keeps the shortcut target strictly above genesis, so `W_(i+1).blue_score ≥ 1 > 0 = L.blue_score` holds trivially.) The terminator anchor's `Inclusion(L.lane_blue_score)` follows from I1: the walk lands in `L`'s window, where the lane is in the SMT with leaf `L.lane_blue_score`. Since blue_score strictly increases along the selected-parent chain, `lane_blue_score` uniquely identifies L's activity block, so the terminator's match on it is unambiguous.

### 5.3 Inter-walk gaps are structurally empty of chain blocks

**Case A: `inactivity_shortcut(W_i) ≠ ZERO_HASH`.** Let T be the target: the latest in-domain ancestor with `T.bs + F < W_i.bs`. By T's maximality, no in-domain ancestor X exists with `T.bs < X.bs < W_i.bs - F`. The Walk-mode bound (§4.2) gives `W_(i+1).bs ≥ T.bs`, so exactly one of two cases holds: either `W_(i+1).bs ≥ W_i.bs - F`, and `W_(i+1)` lies inside W_i's own covered window; or `W_(i+1).bs < W_i.bs - F`, in which case `W_(i+1) = T` (any other ancestor in that range would contradict T's maximality). Either way the gap `(W_(i+1).bs, W_i.bs - F)` contains no in-domain chain block.
...
**Scope assumption: rotation spacing.** `finality_depth` rotates only at hardforks, never at runtime, so rotations are rare; we assume any two are separated by more than the larger `finality_depth` across the change — the expected case for hardfork-spaced changes. Each rotation then has an isolated `O(F)`-wide influence region (the band where the `min`-rule blends the two `F`-eras), with no other boundary inside it. A walk is then intended to decompose into constant-`F` segments (proven in §5.0–§5.6) joined at isolated single-rotation boundaries, with the single-boundary transition argument below applied locally at each boundary, including long inactivity spans that cross several such boundaries. More tightly-spaced schedules are out of scope: they can create narrow `F`-valleys where a single hop may skip a lane evicted under the smaller `F`, defeating the coverage argument.

**Why a boundary needs its own argument.** The constant-`F` membership equivalence does not survive a rotation: because evicted lanes are never rehydrated (I1), a lane evicted under a smaller `F_old` stays absent even if its last activity later falls inside a larger `F_new` window — so "present iff last-activity + F(W) ≥ W.bs" holds only within a constant-`F` segment. Across a boundary the argument reasons from the actual eviction history, under the full §2.2 predicate `A.bs + min(F(A), F(B)) < B.bs`:
...
Together these reduce a boundary crossing to a density argument: the `min`-rule keeps hops small enough through the transition that the lane's eviction interval is observed by some walked anchor, with `W_i`'s lane proof reflecting the actual per-block eviction history (I1, §3). This is an argument rather than a line-by-line proof; full formalization of the single-boundary transition and its composition across boundaries is deferred. For Kaspa's current constant-`F` regime, §5.0–§5.6 applies directly.

## 6. Why this is the right shape

**Minimal L1 footprint**: one new `activity_root` level in the seq_commit composition (the `inactivity_shortcut` hashed with `lanes_root`, §2.1). No changes to lane-update logic, eviction logic, or pruning policy are required — the proposal adds a structural commitment to a value already derivable from L1's per-block state. The §2.2 `min(F(A), F(B))` shortcut predicate is designed to support `finality_depth` rotations in either direction: the constant-`F` proof (§5.0–§5.6) applies directly, and dynamic `F` is addressed in §5.7 under the same rotation-spacing assumption, by arguing that walks decompose into constant-`F` segments plus isolated single-boundary transitions; full formalization of the transition/composition argument is deferred.

**Guest is config-free and forward-compatible**: the `verify_reset` algorithm doesn't reference `F` (or any other L1 parameter). The guest only verifies cryptographic hash relationships and SMT inclusion/non-inclusion proofs. For rotations satisfying §5.7's rotation-spacing assumption and accepted by the L1 consensus rule, the `inactivity_shortcut` field automatically encodes whichever shortcut target L1 computed and guests continue to verify proofs without modification. The implementation cost of rotation is L1-side bookkeeping (per-block `F` lookup, e.g., via a daa_score → F schedule); the guest-side surface is unchanged.
...
The verify_reset algorithm consumes witnesses as inputs; producing them is an L2 implementation concern, not a consensus one. This section is non-normative — none of it is part of the correctness argument, and the retention heuristics below are not claimed to be complete for every future reset shape. It sketches a few practical strategies.
...
2. **Extend the canonical chain upward**: as new chain blocks arrive during inactivity, retain block B's witness whenever `B.inactivity_shortcut` targets a block at or above the L2's deepest retained anchor — i.e., on the selected-parent-chain segment from that anchor up to B (inclusive), so that B is structurally positioned to land on retention via the canonical (all-Shortcut) walk. This keeps the retained set aligned with the chain a future R would traverse via shortcuts.
...
The change alters `seq_state_root` — which now consumes `activity_root` in place of the raw `lanes_root` — and thus the downstream `seq_commit` value carried by every chain block. `seq_commit` is not part of the Kaspa header pre-image, so block hashes are unaffected; only consensus commitments that consume `seq_commit` change. Consensus-critical; must activate at a hard-fork point identified by **`H_daa`** (the activation daa_score threshold).
