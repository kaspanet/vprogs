...
use vprogs_node_l2_bridge::{
    BlockHash, ConnectStrategy, L2Bridge, L2BridgeConfig, L2Event, NetworkType,
};
...
async fn test_bridge_syncs_and_receives_block_events() {
    ...
    let events = timeout(
        ...
        bridge.event_queue().wait_for(|e| matches!(e, L2Event::Synced)),
    )
    .await
    .expect("timeout waiting for Synced event");

    assert!(events.iter().any(|e| matches!(e, L2Event::Synced)));
    ...
    let events = timeout(
        ...
        bridge.event_queue().wait_for(
            |e| matches!(e, L2Event::BlockAdded { block, .. } if block.header.hash == last_hash),
        ),
    )
    .await
    ...
    let block_added_hashes: Vec<_> = events
        ...
        .filter_map(|e| match e {
            L2Event::BlockAdded { block, .. } => Some(block.header.hash),
            _ => None,
        })
        .collect();
...
async fn test_bridge_syncs_from_specific_block() {
    ...
    let config = L2BridgeConfig::default()
    ...
    let mut bridge = L2Bridge::new(config, Some(start_from), 3);
    bridge.start().unwrap();
...
async fn test_bridge_block_contains_transactions() {
    ...
    let events = timeout(
        Duration::from_secs(10),
        bridge.event_queue().wait_for(
            |e| matches!(e, L2Event::BlockAdded { block, .. } if block.header.hash == block_hash),
        ),
    )
    .await
    ...
    let block = events
        .iter()
        .find_map(|e| match e {
            L2Event::BlockAdded { block, .. } if block.header.hash == block_hash => Some(block),
            _ => None,
        })
...
async fn test_bridge_receives_reorg_events() {
    ...
    let _ = timeout(
        Duration::from_secs(10),
        bridge0.event_queue().wait_for(|e| matches!(e, L2Event::Synced)),
    )
    .await
    .expect("timeout waiting for bridge0 sync");
    let _ = timeout(
        Duration::from_secs(10),
        bridge1.event_queue().wait_for(|e| matches!(e, L2Event::Synced)),
    )
    .await
    ...
    let (main_result, fork_result) = tokio::join!(
        timeout(
            ...
            bridge0.event_queue().wait_for(|e| {
                matches!(e, L2Event::BlockAdded { block, .. } if block.header.hash == last_main_hash)
            })
        ),
        timeout(
            Duration::from_secs(10),
            bridge1.event_queue().wait_for(|e| {
                matches!(e, L2Event::BlockAdded { block, .. } if block.header.hash == last_fork_hash)
            })
        ),
    );
    ...
    let reorg_events = timeout(
        Duration::from_secs(10),
        bridge1.event_queue().wait_for(|e| matches!(e, L2Event::Rollback { .. })),
    )
    .await
    .expect("timeout waiting for reorg events");

    // Verify we got a rollback event.
    let has_rollback = reorg_events.iter().any(|e| matches!(e, L2Event::Rollback { .. }));
...
async fn test_bridge_events_ordered_past_to_present() {
    ...
    let blocks: Vec<_> = events
        .iter()
        .filter_map(|e| match e {
            L2Event::BlockAdded { index, block } => Some((*index, block.clone())),
            _ => None,
        })
        .collect();
...
async fn test_bridge_index_tracking() {
    ...
    let indices: Vec<_> = events
        .iter()
        .filter_map(|e| match e {
            L2Event::BlockAdded { index, .. } => Some(*index),
            _ => None,
        })
        .collect();
...
async fn setup_node_with_bridge(
    ...
    let mut bridge = L2Bridge::new(config, last_processed, last_index);
    bridge.start().unwrap();

    let connected = timeout(
        Duration::from_secs(10),
        bridge.event_queue().wait_for(|e| matches!(e, L2Event::Connected)),
    )
    .await;
