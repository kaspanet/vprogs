...
/// Lock-free event queue for L2 events.
#[derive(Clone)]
pub struct EventQueue {
    queue: Arc<SegQueue<L2Event>>,
    notify: Arc<Notify>,
}
...
impl EventQueue {
    ...
    pub(crate) fn push(&self, event: L2Event) {
        self.queue.push(event);
        self.notify.notify_one();
    }

    /// Pops an event from the queue, if available.
    pub fn pop(&self) -> Option<L2Event> {
        self.queue.pop()
    }
    ...
    pub async fn wait_and_pop(&self) -> L2Event {
        loop {
    ...
    pub async fn wait_for<F>(&self, predicate: F) -> Vec<L2Event>
    where
        F: Fn(&L2Event) -> bool,
    {
