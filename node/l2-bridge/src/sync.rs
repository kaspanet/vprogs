...
use crate::{EventQueue, L2BridgeError, L2Event, Result, state::BridgeState};
...
pub async fn fetch_block(client: &Arc<KaspaRpcClient>, hash: BlockHash) -> Result<RpcBlock> {
    client
        .get_block(hash, true)
        .await
        .map_err(|e| L2BridgeError::RpcCall(format!("get_block failed: {}", e)))
}
...
pub async fn perform_initial_sync(
    ...
    let dag_info = client
        .get_block_dag_info()
        .await
        .map_err(|e| L2BridgeError::RpcCall(format!("get_block_dag_info failed: {}", e)))?;
    ...
    event_queue.push(L2Event::Finalized { index: state.initial_index(), hash: start_hash });
    ...
    let virtual_chain = client
        ...
        .await
        .map_err(|e| {
            L2BridgeError::RpcCall(format!("get_virtual_chain_from_block failed: {}", e))
        })?;

    let added_hashes = virtual_chain.added_chain_block_hashes;

    if added_hashes.is_empty() {
        log::info!("L2 bridge: already synced, no new blocks");
        return Ok(last_processed);
    }

    log::info!("L2 bridge: syncing {} blocks from {:?}", added_hashes.len(), last_processed);
    ...
    for &hash in added_hashes.iter() {
        ...
        event_queue.push(L2Event::BlockAdded { index, block: Box::new(block) });
    }
