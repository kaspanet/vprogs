...
use crate::{
    EventQueue, L2BridgeConfig, L2BridgeError, L2Event, Result,
    state::BridgeState,
    sync::{fetch_block, get_block_parents, perform_initial_sync},
};

/// The main L2 bridge that connects to Kaspa L1 and emits simplified events
...
impl L2Bridge {
    /// Creates a new L2 bridge with the given configuration.
    ///
    ...
    pub fn new(config: L2BridgeConfig, last_processed: Option<BlockHash>, last_index: u64) -> Self {
        Self {
            config,
    ...
    pub fn start(&mut self) -> Result<()> {
        if self.handle.is_some() {
            return Err(L2BridgeError::AlreadyStarted);
        }
    ...
    /// Returns the event queue for consuming L2 events.
...
async fn run_event_loop(
    ...
    loop {
        select_biased! {
            ...
            _ = notify_shutdown.notified().fuse() => {
                log::info!("L2 bridge shutdown requested");
                break;
            }
            ...
            msg = rpc_ctl_channel.receiver.recv().fuse() => {
                match msg {
                    Ok(RpcState::Connected) => {
                        log::info!("L2 bridge connected to {}", client.url().unwrap_or_default());
                        ...
                        event_queue.push(L2Event::Connected);
                        ...
                        if needs_initial_sync {
                            ...
                            match perform_initial_sync(&client, last_processed, &state, &event_queue).await {
                                Ok(last_hash) => {
                                    ...
                                    event_queue.push(L2Event::Synced);
                                    needs_initial_sync = false;
                                    log::info!("L2 bridge initial sync complete");
                                }
                                Err(e) => {
                                    ...
                                    {
                                        log::error!(
                                            "L2 bridge: starting block no longer in chain: {}",
                                            e
                                        );
                                        event_queue.push(L2Event::SyncLost {
                                            reason: format!(
                    ...
                    Ok(RpcState::Disconnected) => {
                        log::info!("L2 bridge disconnected");
                        ...
                        event_queue.push(L2Event::Disconnected);
                    }
                    Err(e) => {
                ...
                match notification {
                    Ok(Notification::VirtualChainChanged(vcc)) => {
                        ...
                        if !vcc.removed_chain_block_hashes.is_empty() {
                            log::info!(
                                "L2 bridge: reorg detected, {} blocks removed",
                            ...
                            if let Some(&first_added) = vcc.added_chain_block_hashes.first() {
                                ...
                                if let Ok(parents) = get_block_parents(&client, first_added).await {
                                    if let Some(&common_ancestor) = parents.first() {
                                        log::info!(
                                            "L2 bridge: rolling back to index {} (hash {})",
                                            rollback_index,
                                            common_ancestor
                                        );
                                        ...
                                        event_queue.push(L2Event::Rollback {
                        ...
                        for &hash in vcc.added_chain_block_hashes.iter() {
                            match fetch_block(&client, hash).await {
                                Ok(block) => {
                                    ...
                                    event_queue.push(L2Event::BlockAdded {
                                        index,
                                        block: Box::new(block),
                                    });
                                }
                            ...
                            Ok(dag_info) => {
                                ...
                                if let Some(index) = state.get_index_for_hash(&pruning_hash) {
                                    ...
                                    if index > state.last_finalized_index() {
                                        log::info!(
                                            "L2 bridge: pruning point advanced to index {} (hash {})",
                                            index,
                                            pruning_hash
                                        );
                                        state.set_finalized(index);
                                        event_queue.push(L2Event::Finalized {
                                            index,
                                            hash: pruning_hash,
                                        });
                                    }
                                    ...
                                    log::debug!(
                                        "L2 bridge: pruning point {} not in tracked blocks",
                                        pruning_hash
                                    );
                                }
                            }
    ...
    state.set_connected(false);
    log::info!("L2 bridge worker stopped");
}
