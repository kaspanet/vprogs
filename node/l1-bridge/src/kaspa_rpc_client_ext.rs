...
pub trait KaspaRpcClientExt {
    ...
    fn fetch_block(
        &self,
        hash: BlockHash,
    ) -> impl std::future::Future<Output = Result<RpcBlock, String>> + Send;

    /// Fetches the parent hashes of a block.
    fn fetch_block_parents(
        &self,
        hash: BlockHash,
    ) -> impl std::future::Future<Output = Result<Vec<BlockHash>, String>> + Send;
}

impl KaspaRpcClientExt for KaspaRpcClient {
    async fn fetch_block(&self, hash: BlockHash) -> Result<RpcBlock, String> {
        self.get_block(hash, true).await.map_err(|e| format!("get_block failed: {}", e))
    }

    async fn fetch_block_parents(&self, hash: BlockHash) -> Result<Vec<BlockHash>, String> {
